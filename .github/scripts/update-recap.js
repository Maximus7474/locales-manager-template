...
const { resources, localeLabels, defaultLocaleFile } = require('./data');
...
function generateMdTable(resourceLocaleStatus) {
    ...
    const header = `| Locale ${resources.map(({ name, link }) => `| [${name}](${link})`).join(' ')} |`;
...
for (const resource of localeDirectories) {
    ...
    const files = getLocaleFilesInDirectory(resourceDirectory).filter(file => file !== defaultLocaleFile);

    const templateFile = loadLocaleFile(path.join(resourceDirectory, defaultLocaleFile));
